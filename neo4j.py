...
def get_users_scores(TEST):
    ...
    for i in tqdm(range(NB_USER)):
        ...
        REVIEW_COUNT = int(review_c[0][0]) if len(review_c) > 0 else 0
        ...
        M = int(ambiances[0][0]) if len(ambiances) > 0 else 0
        cats = graph.run(n,params).to_table()
        N = int(cats[0][0]) if len(cats) > 0 else 0
...
#SCORES_TEST5 = get_users_scores(TEST3)




#prendre uniquement les 10 meilleurs users
BEST_SCORES_TEST1 = sorted(SCORES_TEST1.items(), reverse=True , key=operator.itemgetter(1))[:10]

#print(len(SCORES_TEST1))
print(BEST_SCORES_TEST1)
